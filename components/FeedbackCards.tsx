...
function ReadMore({ text }: { text: string }) {
  ...
  const isLong = text?.length > 160; // 適宜調整
  return (
    <div>
      <p
        style={{
      ...
      {isLong && (
        <button
          ...
          style={{ marginTop: 6, fontSize: 12, color: '#2563eb', background: 'none', border: 'none', cursor: 'pointer' }}
        >
          {open ? '閉じる' : 'もっと見る'}
        </button>
      )}
...
export default function FeedbackCards({ feedback }: { feedback: CoachFeedback }) {
  ...
  return (
    <div style={{ display: 'grid', gap: 8 }}>
      {items.map(({ label, key }) => (
        <div key={key} style={{ border: '1px solid #e5e7eb', borderRadius: 10, padding: 12, background: '#fbfbfb' }}>
